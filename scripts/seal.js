/**
 * Seal src/config.json → wxen.enc (WXEN1 + SHA-256 CTR + HMAC).
 *
 *   node scripts/seal.js
 *   node scripts/seal.js --check
 */
...
function loadKey() {
  ...
  if (!raw) throw new Error('Missing WXEN_DB_KEY or .wxen-db.key');
...
function main() {
  ...
  if (!fs.existsSync(srcPath)) {
    throw new Error('Create src/config.json first (see src/config.example.json)');
  }
  const plain = fs.readFileSync(srcPath);
  JSON.parse(plain.toString('utf8'));
  const blob = seal(plain, key);
  const round = open(blob, key);
  if (!round.equals(plain)) throw new Error('round-trip failed');

  if (process.argv.includes('--check')) {
    console.log('[seal] ok', plain.length, 'bytes');
    return;
  }

  const out = path.join(root, 'wxen.enc');
  fs.writeFileSync(out, blob.toString('base64') + '\n');
  console.log('[seal] wrote', path.relative(root, out));
}
